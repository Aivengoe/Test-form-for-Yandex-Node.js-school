...
class MyForm {
	...
	validate() {
		...
		let validEmail = /^[A-Za-z0-9_.+-]+@ya(\.ru|ndex\.(ru|ua|by|kz|com))$/i.test(
			this.email.value
		);
		if (!validEmail) {
			result.isValid = false;
		...
		let validPhone = /^\+7\(\d{3}\)\d{3}-\d{2}-\d{2}$/i.test(
			this.phone.value
		);
		let validPhoneSum =
			this.phone.value.match(/\d/g).reduce((a, b) => +a + +b) <= 30;
	...
	submit() {
		...
		let errors = Array.prototype.slice.call(
			this.form.getElementsByClassName("error")
		);
